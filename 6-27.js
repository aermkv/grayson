//tokenData.hash = '0xde4ebc6e9bbf3bd283b741a5131976553290d2249bc06de4bf82270d22325588'
...
function setup() {
  ...
  chooseDefaultLight()
  howBright = chooseBrightness() 
  console.log('brightness is ' + howBright)
  //bgLayer = createGraphics(smD,smD)
  ...
  if (gridType === circleGrid || shaftGrid || arcGrid || arcLayers) {
    mothsSetup(gridType)
  }
...
function draw() {
  ...
  imageMode(CENTER)
  let cCoeff = map(mouseY,-h(.5),h(.5),topVal,bottomVal)
  background(pal.dark[0]*cCoeff, pal.dark[1]*cCoeff, pal.dark[2]*cCoeff, 83 * howBright)
  shimmerPixels()
  ...
  //background(pal.dark[0]*cCoeff, pal.dark[1]*cCoeff, pal.dark[2]*cCoeff,0)
  //background(30,25)
  drawIrregCircles()
  //drawBlobs()
  ...
  //background(pal.dark[0]*cCoeff, pal.dark[1]*cCoeff, pal.dark[2]*cCoeff,145)
...
function chooseBrightness() {
  const brightnessPercent = map(decPairs[12],0,255,0,1);
...
function chooseTexture(grid) {
  const texturePercent = map(decPairs[9],0,255,0,1);
    if (texturePercent < .5) {
      ...
      vanish_P(.001, pal.accent_light)
    }
}

function chooseDefaultLight() {
  const defaultLightPercent = map(decPairs[11],0,255,0,1);
  if (defaultLightPercent < .5) {
    topVal = 1, bottomVal = 0
  }else{
    topVal = 0, bottomVal = 1
  }
...
function blurredEllipse_Sp(x,y,size,r,g,b,a) {
  for (let i = 0; i < 4; i++){ // reduced to be lighter
    let off = w(.005)
    spirals.fill(r,g,b,a*i*.1)
    spirals.noStroke()
    spirals.ellipse(x,y,size*(.2*i))
  }
}
...
function triangle_rgb(x1,y1,x2,y2,x3,y3,r,g,b,a) {
  fill(r,g,b,a)
...
function blurredEllipse_Sp(x,y,size,r,g,b,a) {
  for (let i = 0; i < 12; i++){
    ...
    spirals.ellipse(x,y,1.6*size - (size/10)*i)
    //polygon(spirals, x+o,y+o,1.6*size - (size/10)*i,rnd(5))
  }
}

function drawCirc_Vert_SOLIDS(radius, t_offset, limit, r, g, b, a, grid) {
  push()
  spirals.strokeWeight(3)
  spirals.fill(255)
  spirals.noStroke()
  let xoff = 0
  ...
  let speed_coeff = map(radius,0,w(1.3),2,1)
  let rad_mult = map(frameCount,0,500,1,1.8)

  let x = spirals.width/2 + radius * rad_mult * 1.25 * cos(speed_coeff*t+t_offset) + map(sin(t),-1,1,-w(.03),w(.03))
  let y = spirals.height/2 + radius * rad_mult * 1.25 * sin(speed_coeff*t+t_offset) + rnd(-w(.005),w(.005))
  if (grid == 'arcGrid' || grid == 'arcLayers') {
    x = spirals.width/2 + radius * rad_mult * 2.45 * cos(speed_coeff*t+t_offset) + map(sin(t),-1,1,-w(.03),w(.03))
    y = (spirals.height/2 + (radius + w(.02)) * rad_mult * 2.8 * sin(speed_coeff*t+t_offset) + rnd(-w(.005),w(.005))) * 1.5 + h(.2)
  }
  if (grid == 'shaftGrid') {
    y = (spirals.height/2 + radius * rad_mult * sin(speed_coeff*t+t_offset) + rnd(-w(.005),w(.005))) * 3.5 - h(1.25)
  }
  let size = map(sin(t+180),-1,1,w(.04),w(.1)) + map(dist(x,y,width/2,height/2),0,w(1),0,w(.03))

  //if (frameCount < limit) {
    //let coeff = map(dist(x,y,spirals.width/2,spirals.height/2),0,w(.9),1,.3)
    let alpha_coeff = map(dist(x,y,width/2,height/2),0,w(1),1,2)
...
function drawSpirals_SOLIDS(r, g, b, a, grid) {
  ...
  if (frameCount > begin) {
    for (let i = 0; i < num_circs; i++) {
      let rad = w(.005)+i*w(.03)
      if (grid == 'arcGrid' || grid == 'arcLayers') {
        rad = w(.015)+i*w(.025)
      }
      let coeff = map(rad,0,w(.005)+num_circs*w(.03),1,0)
...
function bigBrushTexture(grid) {
  if (frameCount < 500) {
    drawSpirals_SOLIDS(pal.accent_light[0], pal.accent_light[1], pal.accent_light[2], 45, grid)
...
function texturedVanishingPoint(init_distort,line_val,size,inc,diff,max_distort) {
  push()
  //translate(width/2,0)
...
function vanish_P(line_val, col) {
  push()
  graphics.blendMode(graphics.OVERLAY)
  graphics.noFill()
  ...
  for (let i = 0; i < num_circs; i++) {
    ...
    texturedVanishingPoint(i_d,line_val,size,inc,0,w(.06))
    texturedVanishingPoint(i_d,line_val,size,inc*15,30,w(.06))
    texturedVanishingPoint(i_d,line_val,size,inc*25,60,w(.06))
    graphics.stroke(col[0] * c, col[1] * c, col[2] * c, 38 * alpha_coeff)
    texturedVanishingPoint(i_d,line_val,size,inc*15,90,w(.06))
    texturedVanishingPoint(i_d,line_val,size,inc*25,120,w(.06))
    graphics.stroke(col[0] * c, col[1] * c, col[2] * c, 15 * alpha_coeff)
    texturedVanishingPoint(i_d,line_val,size,inc*15,270,w(.06))
    texturedVanishingPoint(i_d,line_val,size,inc*25,300,w(.06))
    xoff += .01
  }
  pop()
}
...
function irregExpand(radius,noiseVal,grid) {
  ...
  for (let a = 0; a <= TWO_PI; a += radians(6)) {
    ...
    //ellipse(x, y, rnd(w(.01)));
    //ellipse(x*1.5, y*1.5, rnd(w(.01)));
    //ellipse(x*1.85, y*1.85, rnd(w(.01)));
    vertex(x,y)
    //line(x,y,x2,y2)
    //line(x2,y2,x3,y3)
  }
...
function irregExpandShimmer(radius,noiseVal,grid) {
  ...
  for (let a = 0; a <= TWO_PI - radians(3); a += radians(6)) {
    ...
    //blurredEllipse(x*2.5, y*2.5, shimmerSize, pal.accent_dark[0], pal.accent_dark[1], pal.accent_dark[2], 85)
    //vertex(x,y)
    //line(x,y,x2,y2)
    //line(x2,y2,x3,y3)
  }
...
class IrregCircle{
  ...
  show() {
    ...
    if (this.expandType === 'ellipses'){
      ...
      stroke(pal.light[0],pal.light[1],pal.light[2],20)
      strokeWeight(w(.002)) 
    ...
    if (this.expandType === 'ellipses-rose'){
      ...
      let alphaVal = 1
      if (this.radius > w(.15)) {
        alphaVal = map(this.radius, w(.15), w(.5), 1, 0)
      }
      let sW_Val = map(this.radius, 0, w(.65), .5, 2.5)
      stroke(pal.light[0],pal.light[1],pal.light[2],85 * alphaVal)
      strokeWeight(w(.005)*sW_Val) 
      background(pal.dark[0], pal.dark[1], pal.dark[2], 5)
      ...
      stroke(pal.light[0],pal.light[1],pal.light[2],25 * alphaVal)
      strokeWeight(w(.01)*sW_Val) 
      irregExpand(this.radius,nV,this.grid)
...
function setInwardParticles(grid) {
  numP_inwards = 300
  for (let i = 0; i < numP; i++) {
    let radius = rnd(w(.2),w(.68));
    let angle = rnd(TWO_PI)
    let size = rnd(w(.01))
    //let diff = rnd(.85,1.15)
    let speedCoeff = rnd(1.5,3.5)
    let x = radius*cos(angle)
    let y = radius*sin(angle)
    let p = new InwardParticle(x,y,size,radius,angle,speedCoeff,grid)
    inwardParticles.push(p);
  }
}

function drawInwardParticles() {
  for (let i = 0; i < inwardParticles.length; i++){
    inwardParticles[i].run();
  }
}

class InwardParticle{
  constructor(_x,_y,_size,_radius,_angle,_speedCoeff,_grid){
    this.x = _x;
    this.y = _y;
    this.size = _size;
    this.radius = _radius;
  ...
  checkEdges() {
    if (dist(this.x,this.y,0,0) < 10) {
      let newRadius = rnd(w(.35),w(.68))
  ...
  show() {
    ...
    noStroke()
    let c = map(this.radius,0,w(.7),1,.6)
    fill(pal.accent_light[0]*c,pal.accent_light[1]*c,pal.accent_light[2]*c,135)
    let y_mult = 1
    if (this.grid == 'shaftGrid') {
      y_mult = 2
    }
    ellipse(this.x,this.y * y_mult,this.size)
    pop()
  }
}
...
function fracturedSun() {
  ...
  for (let blobNum = 0; blobNum < 10; blobNum++){
    size_inc = w(.003)
    blobSize = map(blobNum,0,10,w(1),w(.035))
    if (blobSize > w(1)){
      blobSize = w(.025)
    }
    let fadeIn = 1
    if (frameCount < 100) {
      fadeIn = map(frameCount,0,100,0,1)
    }
    fill(map(blobNum,0,10,pal.dark[0],pal.light[0]),map(blobNum,0,10,pal.dark[1],233),map(blobNum,0,10,pal.light[2],pal.dark[2]),map(blobNum,0,10,3,10)*fadeIn)
    let angle = radians(frameCount / 200) + radians(blobNum*15)
    beginShape();
    for(var i = 0, j = 0; i < TWO_PI; i += TWO_PI/slices, j++){
      curveVertex(sin(i) * blobSize + map(cos(millis()/randomWeights[j]), -1, 1, -w(.1), w(.1)), cos(i) * blobSize + map(sin(millis()/randomWeights[j+ slices]), -1, 1, -w(.1), w(.1)));
    }
    //Needed to make the blob correctly shaped.
    //curveVertex(sin(0) * blobSize + map(cos(millis()/randomWeights[0]), 0, 1, -30, 30), cos(0) * blobSize + map(sin(millis()/randomWeights[0+ slices]), 0, 1, -20, 20));
    //curveVertex(sin(TWO_PI/slices) * blobSize + map(cos(angle), 0, 1, -30, 30), cos(TWO_PI/slices) * blobSize + map(sin(angle), 0, 1, -20, 20));
    //curveVertex(sin(2*TWO_PI/slices) * blobSize + map(cos(angle), 0, 1, -30, 30), cos(2*TWO_PI/slices) * blobSize + map(sin(angle), 0, 1, -20, 20));
    endShape(CLOSE);
    blobSize += size_inc
  }
  pop()
...
function wallOfFire() {
  ...
  for (let blobNum = 0; blobNum < 10; blobNum++){
    ...
    fill(map(blobNum,0,10,pal.accent_dark[0],pal.accent_light[0]),map(blobNum,0,10,pal.accent_dark[1],233),map(blobNum,0,10,pal.accent_light[2],pal.accent_dark[2]),map(blobNum,0,10,3,10)*fadeIn)
    let angle = radians(frameCount / 200) + radians(blobNum*15)
    beginShape();
    for(var i = 0, j = 0; i < TWO_PI; i += TWO_PI/slices, j++){
      curveVertex(sin(i) * blobSize + map(cos(millis()/3/randomWeights[j]), -1, 1, -w(.1), w(.1)), cos(i) * blobSize + map(sin(millis()/3/randomWeights[j+ slices]), -1, 1, -w(.1), w(.1)) * 4);
    }
    endShape(CLOSE);
    blobSize += size_inc
  }
...
function arcLayers() {
  ...
  for (let i = 0; i < numPoints; i++) {
    let angle = map(i,0,numPoints,PI-radians(15),TWO_PI+radians(20)) + rnd(radians(3))
    let angle2 = map(i,0,numPoints/3,PI-radians(15),TWO_PI+radians(20))
    let radius = w(.25)
    let radius2 = w(.32)
    let radiusLG = w(.29)
...
function moth_9(x,y,sc,rot,fadeIn,metl) {
  ...
  let rotOption = rot
  if (mouseDist < w(.4)) {
    rotOption = map(mouseDist,w(.4),0,rot,Math.atan2(mouseY-y, mouseX-x) - radians(90));
  }
  rotate(rotOption)
  scale(sc)
