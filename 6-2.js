...
function setBgDaubs() {
  ...
  for (let i = 0; i < num_dots; i++){
    ...
    for (let j = 0; j < num_dots; j++){
      ...
      blurredEllipse_bg(x,y,default_size * noise(xoff + 2000, yoff + 2000),col[0]*colorCoeff, col[1]*colorCoeff, col[2]*colorCoeff, 60)
      yoff += inc
    }
    xoff += inc
  }
...
function setWater() {
  ...
  for (let y = 0; y < bgNumRows * 2; y++) {
    ...
    for (let x = 0; x < bgNumRows * 2; x++) {
      ...
      p = new ParticleWater(xPos,yPos,w(.03),col,20);
...
function setMovingBackground() {
  ...
  for (let i = 0; i < numP; i++){
    ...
    p = new Particle(radius,w(.06),colVals,w(.15),20);
    particles.push(p);
  }
  numP_Out = 110;
...
function setInParticles2() {
  ...
  for (let i = 0; i < numP_In2; i++) {
    ...
    let c = rnd(255)
    let col = [c,c,c]
    let p = new ParticleIn2(x,y,size,radius,angle,speedCoeff,col)
    particlesIn2.push(p);
  }
}
...
function chooseMoth() { 
  const mothOptions = [moth_5, moth_6, moth_7, moth_8];
...
class Buttermoth{
  ...
  update(fM_xoff,fM_yoff){
    ...
    fill(c * this.met[0], c * this.met[1], c * this.met[2], map(frameCount,this.start,this.start+10,0,90) + c * 1.2);
    noStroke()
