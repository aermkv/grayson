...
function draw() {
  ...
  moths.pointLight(78, 57, 39, -w(.5), -h(.5), w(.03))
  moths.pointLight(78, 57, 39, w(.5), h(.5), w(.03))
  moths.pointLight(78, 57, 39, w(.5), -h(.5), w(.03))
  moths.pointLight(78, 57, 39, -w(.5), h(.5), w(.03))
...
function chooseExpandType() {
  const expandTypePercent = map(decPairs[6],0,255,0,1);
    if (expandTypePercent < .25) {
      expandType = 'normal'
    }else if (expandTypePercent < .5){
      expandType = 'intense'
    }else if (expandTypePercent < .75){
...
function chooseMoth() { 
  const mothOptions = [moth_9,moth_10,moth_11];
...
function moth_10(x,y,sc,rot) {

  moths.push()
  //moths.resetMatrix()
  //translate(-width/2,-height/2)
  moths.translate(x,y)
  moths.rotateZ(HALF_PI + rot)
  moths.scale(sc)
...
function moth_11(x,y,sc,rot) {

  moths.push()
  //moths.resetMatrix()
  //translate(-width/2,-height/2)
  moths.translate(x,y)
  moths.rotateZ(PI + rot)
  moths.scale(sc)
...
class Buttermoth_Spiral{
  ...
  update() {
    ...
    this.x *= map(t,0,50,1,.5)
    this.y *= map(t,0,50,1,.5)
