...
function draw() {
  ...
  background(pal.dark[0]*cCoeff, pal.dark[1]*cCoeff, pal.dark[2]*cCoeff,45)
...
function chooseExpandType() {
  const expandTypePercent = map(decPairs[6],0,255,0,1);
    if (expandTypePercent < .11) {
      expandType = 'normal'
    }else if (expandTypePercent < .22){
      expandType = 'intense'
    }else if (expandTypePercent < .33){
      expandType = 'intense-white'
    }else if (expandTypePercent < .44){
      expandType = 'ellipses'
    }else if (expandTypePercent < .55){
      expandType = 'spirals'
    }else if (expandTypePercent < .66){
      expandType = 'spirals-smooth'
    }else if (expandTypePercent < .77){
      expandType = 'ellipses-shimmer'
    }else if (expandTypePercent < .88){
      expandType = 'nebula'
    }else{
      expandType = 'strobe'
    }
...
function chooseGrid() {
  const gridPercent = map(decPairs[3],0,255,0,1);
    ...
    }else if (gridPercent < .4) {
      chosenGrid = arcLayers
    }else if (gridPercent < .6) {
      chosenGrid = arcGrid
    }else if (gridPercent < .8) {
      chosenGrid = arcLayers
    }else{
...
function irregExpandShimmer(radius,noiseVal) {
  //beginShape();
  for (let a = 0; a <= TWO_PI - radians(3); a += radians(6)) {
    ...
    let x = radius * diff * cos(a);
    let y = radius * diff * sin(a);
    shimmerSize = map(sin(frameCount / 10 + a),-1,1,0,w(.01))
...
function linearCircs(r,nV) {
  irregCircle(r,nV)
  irregCircle(r*1.33,nV*2)
  irregCircle(r*1.66,nV*3)
}
...
class IrregCircle{
  ...
  show() {
    ...
    if (this.expandType === 'normal'){
      stroke(200,2)
      strokeWeight(w(.003))
      linearCircs(this.radius,this.noiseVal)
    }
    ...
    if (this.expandType === 'ellipses-shimmer'){
      ...
      //strokeWeight(3) 
      noStroke()
      irregExpandShimmer(this.radius,this.noiseVal)
      pop()
    }
...
function setInwardParticles() {
  ...
  for (let i = 0; i < numP; i++) {
    ...
    let p = new InwardParticle(x,y,size,radius,angle,speedCoeff)
...
class InwardParticle{
  constructor(_x,_y,_size,_radius,_angle,_speedCoeff){
    this.x = _x;
    this.y = _y;
    this.size = _size;
    this.radius = _radius;
...
function fracturedSun() {
  ...
  for (let blobNum = 0; blobNum < 10; blobNum++){
    ...
    fill(map(blobNum,0,10,pal.dark[0],pal.light[0]),map(blobNum,0,10,pal.dark[1],233),map(blobNum,0,10,pal.light[2],pal.dark[2]),map(blobNum,0,10,3,13)*fadeIn)
    let angle = radians(frameCount / 200) + radians(blobNum*15)
    beginShape();
    for(var i = 0, j = 0; i < TWO_PI; i += TWO_PI/slices, j++){
      curveVertex(sin(i) * blobSize + map(cos(millis()/randomWeights[j]), 0, 1, -30, 30), cos(i) * blobSize + map(sin(millis()/randomWeights[j+ slices]), 0, 1, -20, 20));
    }
...
function arcLayers() {
  ...
  for (let i = 0; i < numPoints; i++) {
    let angle = map(i,0,numPoints,PI-radians(7),TWO_PI+radians(12))
    let angle2 = map(i,0,numPoints/3,PI-radians(7),TWO_PI+radians(12))
    let radius = w(.25)
    let radius2 = w(.32)
    let radiusLG = w(.29)
    ...
    let y = radius * sin(angle) + h(.28) + smallRandomOffsetY
    let x2 = radius2 * cos(angle) + smallRandomOffsetX
    let y2 = radius2 * sin(angle) + h(.28) + smallRandomOffsetY
    let x3 = radiusLG * cos(angle2)
    let y3 = radiusLG * sin(angle2) + h(.28)
    let compressionMult = map(dist(x,0,0,0),0,w(.5),1,2.9)
    let compressionSubtract = map(dist(x,0,0,0),0,w(.5),w(.25),0)
    let point = createVector(x,y*compressionMult - compressionSubtract)
...
function moth_9(x,y,sc,rot,fadeIn) {
...
function moth_10(x,y,sc,rot,fadeIn) {
...
function moth_11(x,y,sc,rot,fadeIn) {
...
function moth_12(x,y,sc,rot,fadeIn) {
...
class Buttermoth{
  ...
  update(fM_xoff,fM_yoff){
    ...
    if (this.grid == arcLayers) {
      this.moth(this.loc.x * breathing, this.loc.y, this.mothScale, this.rot + map(sin(frameCount/90 + this.angleOffset),-1,1,0,this.rot),this.start);
    }else{
      this.moth(this.loc.x * breathing, this.loc.y * breathing, this.mothScale, this.rot + map(sin(frameCount/90 + this.angleOffset),-1,1,0,this.rot),this.start);
    }
  }
}
...
class Buttermoth_Spiral{
  ...
  show(fM_xoff,fM_yoff) {
    ...
    noStroke()
    background(0,1.3)
    this.moth(this.x, this.y, this.mothScale * map(dist(this.x,this.y,0,0),w(.8),0,1.3,.45), map(sin(frameCount/60),-1,1,0,this.rot),this.start);
  }
}